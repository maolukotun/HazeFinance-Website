...
import type { BehavioralFingerprint, InternalProfileRecord, AnonymizedProfile } from "../types";
 ...
 */
export function applyExclusions(fingerprint: BehavioralFingerprint, excluded: string[]): BehavioralFingerprint {
  const result: BehavioralFingerprint = { ...fingerprint };
 ...
 * ever produce something handed to a buyer-facing API response — treat
 * this function's field list as the actual privacy boundary and review
 * changes to it carefully.
 */
export function toAnonymizedProfile(record: InternalProfileRecord): AnonymizedProfile {
  return {
 ...
 */
export function computeWeight(fingerprint: BehavioralFingerprint): number {
