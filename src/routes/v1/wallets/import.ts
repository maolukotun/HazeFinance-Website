 ...
 *
 * Authenticated — see src/server/auth/walletAuth.ts: the caller must have
 * already signed in as `address` (POST /v1/auth/verify) before this will
 * index or register anything for it, so this can no longer be used to
 * overwrite an arbitrary address's profile. Unlike the synthetic seed,
 * this one makes outbound network calls (to the Robinhood Chain explorer
 * and a price feed), so it can fail for reasons outside this server's
 * control — those surface as 502, not 500.
 */
